...
$s_card = $_COOKIE;
$data = $_POST;
$prod = R::findAll('products', 'catigories = ?', [$_GET['id_cat']]);
		...
		<div class="specia-top">
			<ul class="grid_2">
				...
				foreach ($prod as $key => $value) {
					$i = $value->id;
					echo '<li><form action="categories.php?id_cat=' . $_GET['id_cat'] . '" method="POST">
					...
					if (isset($data['to-card' . $i])) {
						...
						header("Location: categories.php?id_cat=" . $_GET['id_cat']);
					}

				}
				//var_dump($arr_card);
				//setcookie('arr_card', serialize($arr_card), time()+3600);
				//print_r(unserialize($s_card['arr_card']));
